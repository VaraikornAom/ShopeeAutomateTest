...
describe.only('Shopee Tests', () => {
  before(() => {
    ...
    // Perform Google search for Shopee
    cy.get('textarea[name="q"]').type('Shopee{enter}');
    cy.wait(getRandomDelay());

    // Click the div containing "Shopee Thailand" text
    cy.contains('div', 'Shopee Thailand').click();
    cy.wait(getRandomDelay());

    // Handle the navigation to Shopee
    cy.origin('https://shopee.co.th', () => {
      // Check if the modal appears and handle it
  ...
  it('ScenarioC: Website display search result as a list', () => {
    cy.origin('https://shopee.co.th', () => {
      cy.wait(getRandomDelay());
